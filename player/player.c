...
#define DEFAULT_AV_SYNC_TYPE AV_SYNC_AUDIO_MASTER //以音频为参考进行同步
...
int synchronize_audio(VideoState *is, short *samples,int samples_size)  //音频同步到视频，暂时不用
{
  int n;
  double ref_clock;
...
int audio_decode_frame(VideoState *is, uint8_t *audio_buf, int buf_size)
{
  ...
  for(;;) {
    ...
    if(pkt->data)
      av_free_packet(pkt);  //释放在av_read_frame申请的内存
...
//SDL回调函数，调用audio_decode_frame解码音频帧，并通过SDL_MixAudio播放音频
    ...
    while(len > 0)  //大于0表示声卡缓冲区有空位
    {
        if(is->audio_buf_index >= is->audio_buf_size) //若外部的缓冲区数据已经发完，再从队列读包解码填充缓冲区，audio_buf_index和audio_buf_size初始值0
        {
            ...
            if(audio_size < 0) {
                ...
                audio_size = synchronize_audio(is, (int16_t *)is->audio_buf,audio_size);    //采用视频同步到音频策略时，函数不起作用，直接返回audio_size
...
//XXXms后调用回调函数sdl_refresh_timer_cb
...
{
  fprintf(stderr,"%5.2f%%\n",100*(is->video_clock/(global_video_state->duration/1000000))); //打印播放进度
  SDL_Rect rect;
  VideoPicture *vp;
//  float aspect_ratio;
  int w, h;
...
//反复调用schedule_refresh来实现视频帧刷新，同时通过delay来实现音视频同步
void video_refresh_timer(void *userdata)
{
    ...
    if(is->video_st)    //判断视频流有无错误，如源视频流缺漏（只有音频）或格式错误等情况
    {
        if(is->pictq_size == 0) {
             ...
             * 度差值，不能超过视频帧间隔，如果超了，比如音频播放比视频快，那就要加
